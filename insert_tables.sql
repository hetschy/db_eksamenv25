...
CREATE TABLE loype(
	...
	Navn VARCHAR(50) NOT NULL,
	FOREIGN KEY (DestinasjonID) REFERENCES destinasjon(DestinasjonID)
);
...
CREATE TABLE spor(
	...
	FOREIGN KEY (LoypeID) REFERENCES loype(LoypeID),
	FOREIGN KEY (SportypeID) REFERENCES sportype(SportypeID)
);
...
CREATE TABLE loypeforhold(
	...
	FOREIGN KEY (SnotypeID) REFERENCES snotype(SnotypeID)
);
...
CREATE TABLE smoring(
	...
	FOREIGN KEY (Produktkategori) REFERENCES produktkategori(ProduktkategoriID)
);
...
CREATE TABLE produktegenskaper(
	...
	FOREIGN KEY (SmoringID) REFERENCES smoring(SmoringID),
	FOREIGN KEY (BrukernivaID) REFERENCES brukerniva(BrukernivaID),
	FOREIGN KEY (SnotypeID) REFERENCES snotype(SnotypeID),
